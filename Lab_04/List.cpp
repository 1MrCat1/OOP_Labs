...
/*template <template <class,class,class> class T, class A, class B, class C> void List<T,A,B,C>::Add_figure(std::shared_ptr <T<Hexagon>> &&figure) {
	std::shared_ptr <T<Hexagon>> item = figure;
	item->SetNext(this->head->GetNext());
	this->head->SetNext(item);
	this->size++;
}

template <template <class,class,class> class T, class A, class B, class C> void List<T,A,B,C>::Add_figure(std::shared_ptr <T<Octagon>> &&figure) {
	std::shared_ptr <T<Octagon>> item = figure;
	item->SetNext(this->head->GetNext());
	this->head->SetNext(item);
	this->size++;
}*/
...
/*template <template <class,class,class> class T,class A,class B,class C> void List<T,A,B,C>::Remove_figure(const T<A, B, C>& fig) {
	std::shared_ptr<ListItem<T>> prev = this->head;
	std::shared_ptr<ListItem<T>> iter = this->head;
	while (iter != nullptr) {
		if (iter->GetFigure() != nullptr) {
			if (*iter->GetFigure() == fig) {
				if (iter == this->head) {
					this->head = iter->GetNext();
					iter->SetNext(nullptr);
					iter->~ListItem();
...
/*template <template <class,class,class> class T,class A,class B,class C> void List<T,A,B,C>::Remove_figure(const T<Octagon>& fig) {
	std::shared_ptr<ListItem<T>> prev = this->head;
	std::shared_ptr<ListItem<T>> iter = this->head;
	while (iter != nullptr) {
		if (iter->GetFigure() != nullptr) {
